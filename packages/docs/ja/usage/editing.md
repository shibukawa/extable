...
Extableはスキーマのフックで、外部サービス/UIと連携した「リッチ編集」を実現できます。

- **リモートLookup（タイプアヘッド）:** 入力中に候補を取得してドロップダウン表示し、安定IDなどを含む構造化値を保存します。
- **外部エディタ委譲:** モーダル/リッチエディタUIを開き、Promiseの結果でcommit/cancelできます。
- **ホバー・ツールチップ（非同期）:** ホバー時のツールチップ文字列を同期/非同期で計算できます。

インタラクティブデモ: [リッチ編集（Lookup / 外部エディタ / ツールチップ）](/ja/demos/rich-editing-remote)
