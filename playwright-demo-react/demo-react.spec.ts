...
test("demo-react: search and filter/sort sidebars open", async ({ page }) => {
  await page.goto("/");

  await page.locator('input[name="mode"][value="html"]').check();
  ...
  await expect(page.locator(".extable-search-sidebar")).toHaveCount(0);
  ...
  await expect(page.locator(".extable-filter-sort-sidebar")).toHaveCount(0);
});
